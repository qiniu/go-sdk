...
type jsonGetIamUserPolicy struct {
...
func (j *GetIamUserPolicy) MarshalJSON() ([]byte, error) {
	if err := j.validate(); err != nil {
		return nil, err
	}
...
// 返回的 IAM 子账号的授权策略列表
type GetIamUserPolicies = []GetIamUserPolicy

// IAM 子账号的授权策略信息
type Data struct {
	Count int64              // IAM 子账号的授权策略数量
	List  GetIamUserPolicies // IAM 子账号的授权策略列表
}
