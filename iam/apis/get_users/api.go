...
type Response struct {
	Data GetIamUsersData // IAM 子账号信息
}

// 返回的 IAM 子账号
type GetIamUser struct {
	Id            string // 记录 ID
...
func (j *GetIamUser) MarshalJSON() ([]byte, error) {
	if err := j.validate(); err != nil {
		return nil, err
	}
	return json.Marshal(&jsonGetIamUser{Id: j.Id, RootUid: j.RootUid, Iuid: j.Iuid, Alias: j.Alias, CreatedAt: j.CreatedAt, UpdatedAt: j.UpdatedAt, LastLoginTime: j.LastLoginTime, Enabled: j.Enabled})
}
func (j *GetIamUser) UnmarshalJSON(data []byte) error {
	var nj jsonGetIamUser
...
func (j *GetIamUser) validate() error {
	if j.Id == "" {
...
// 返回的 IAM 子账号列表
type GetIamUsers = []GetIamUser

// IAM 子账号信息
type Data struct {
	Count int64       // IAM 子账号数量
	List  GetIamUsers // IAM 子账号列表
}

// 返回的 IAM 子账号列表信息
type GetIamUsersData = Data
type jsonData struct {
	Count int64       `json:"count"` // IAM 子账号数量
	List  GetIamUsers `json:"list"`  // IAM 子账号列表
}
...
// 返回的 IAM 子账号列表响应
type GetIamUsersResp = Response
type jsonResponse struct {
	Data GetIamUsersData `json:"data"` // IAM 子账号信息
}
