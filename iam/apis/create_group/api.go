...
type Request struct {
	...
	Alias       string                          // 用户分组别名，由 `A-Za-z0-9` 组成
	Description string                          // 用户分组描述
}

// 创建用户分组参数
type CreateGroupParam = Request
type jsonRequest struct {
	Alias       string `json:"alias,omitempty"`       // 用户分组别名，由 `A-Za-z0-9` 组成
...
func (j *Request) MarshalJSON() ([]byte, error) {
	...
	return json.Marshal(&jsonRequest{Alias: j.Alias, Description: j.Description})
}
...
// 返回的用户分组响应
type CreatedGroupResp = Response
type jsonResponse struct {
	Data CreatedGroupData `json:"data"` // 用户分组信息
}
